	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Bem vindo, n esqueça d mama o dark @${num.split('@')[0]}\nSelamat datang di group *${mdata.subject}*`
			...
			switch(command) {
				...
				case 'tp':
					if (args.length < 1) {
						...
						return reply('O texto também por favor tio ')
					}
					reply(mess.wait)
					...
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('error', function (err) {
								...
								reply(`❌ Falhou, pada saat mengkonversi ${tipe} ke stiker`)
							})
							.on('end', function () {
								console.log('Finish')
								exec(`webpmux -set exif ${addMetadata('BOT', authorname)} ${ran} -o ${ran}`, async (error) => {
				...
				case 'demote':
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Parou de mama e perdeu adm\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
				...
				case 'toimg':
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						fs.unlinkSync(media)
						if (err) return reply('❌ deu error, tenta dnv gay ❌')
				...
				case 'welcome':
					...
					if (args.length < 1) return reply('Hmmmm')
					if (Number(args[0]) === 1) {
						if (isWelkom) return reply('Ja ativo, dark dlc')
						welkom.push(from)
